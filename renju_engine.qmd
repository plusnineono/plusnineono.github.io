...
<style>
  .renju-app {
    ...
    --line: #8b5a2b;
    --board: #d8b574;
    --board-size: min(72vh, 680px);
    color: var(--text);
    font-family: Inter, system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif;
    display: block;
  }
  .renju-app * { box-sizing: border-box; }
  .renju-app .wrap { display: grid; grid-template-columns: minmax(0, var(--board-size)) minmax(320px, 360px); gap: 16px; padding: 4px 0 14px; max-width: 1100px; margin: 0 auto; align-items: start; }
  .renju-app .panel { background: linear-gradient(180deg, var(--panel), var(--panel2)); border-radius: 18px; box-shadow: 0 10px 30px rgba(0,0,0,0.25); padding: 14px; }
  .renju-app h1 { font-size: 24px; margin: 0; color: var(--text); }
  .renju-app .row { display: flex; gap: 10px; flex-wrap: wrap; margin-top: 12px; }
  .renju-app button, .renju-app select, .renju-app input { background: #0f1720; color: var(--text); border: 1px solid #3d4a5d; border-radius: 12px; padding: 10px 12px; font-size: 14px; }
  .renju-app button:hover { filter: brightness(1.1); cursor: pointer; }
  .renju-app button.primary { background: #1b4d8a; }
  .renju-app .boardBox { overflow: visible; }
  .renju-app canvas { display: block; width: min(100%, var(--board-size)); max-width: 100%; aspect-ratio: 1 / 1; background: var(--board); border-radius: 18px; border: 2px solid #9d7741; cursor: pointer; touch-action: manipulation; margin: 0 auto; }
  .renju-app .infoGrid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-top: 14px; }
  .renju-app .card { background: rgba(0,0,0,0.18); border-radius: 14px; padding: 10px 12px; }
  .renju-app .card .label { color: var(--muted); font-size: 12px; margin-bottom: 6px; }
  .renju-app .card .value { font-size: 15px; font-weight: 600; }
  .renju-app .resultBanner { margin-top: 14px; padding: 14px 16px; border-radius: 14px; font-weight: 800; font-size: 18px; letter-spacing: 0.02em; text-align: center; background: rgba(255, 209, 102, 0.14); border: 1px solid rgba(255, 209, 102, 0.35); color: #ffe29a; display: none; }
  .renju-app .resultBanner.show { display: block; }
  .renju-app .resultBanner.win-black { background: rgba(17, 19, 21, 0.9); border-color: rgba(255,255,255,0.16); color: #f5f7fa; }
  .renju-app .resultBanner.win-white { background: rgba(240, 244, 248, 0.95); border-color: rgba(120,130,145,0.35); color: #18212b; }
  .renju-app .winRateBox { background: rgba(0,0,0,0.2); border-radius: 14px; padding: 14px; min-height: 140px; display: grid; place-items: center; text-align: center; }
  .renju-app .winRateBar { width: min(100%, 300px); height: 26px; border-radius: 999px; overflow: hidden; display: flex; background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.08); box-shadow: inset 0 1px 3px rgba(0,0,0,0.22); }
  .renju-app .winRateFillBlack { background: linear-gradient(90deg, #495668, #9fb0c5); transition: width 180ms ease; }
  .renju-app .winRateFillWhite { background: linear-gradient(90deg, #ffffff, #d8dee7); transition: width 180ms ease; }
  .renju-app .winRateLegend { width: min(100%, 300px); display: flex; justify-content: space-between; margin-top: 10px; font-size: 13px; color: var(--muted); }
  .renju-app .winRateValue { font-size: 38px; font-weight: 800; line-height: 1; margin-top: 14px; }
  .renju-app .cand { margin-top: 10px; font-size: 13px; line-height: 1.55; }
  .renju-app .hidden { display: none; }
  .renju-app .footer { margin-top: 12px; font-size: 12px; color: var(--muted); line-height: 1.45; }
  .renju-app .info-tip { position: relative; display: inline-flex; align-items: center; cursor: help; color: var(--muted); font-size: 13px; font-weight: 400; }
  .renju-app .info-tip-text { visibility: hidden; opacity: 0; background: #1b2430; color: var(--text); font-size: 12px; border-radius: 10px; padding: 9px 12px; width: 230px; position: absolute; left: 50%; transform: translateX(-50%); bottom: calc(100% + 8px); border: 1px solid #3d4a5d; line-height: 1.5; transition: opacity 0.15s; z-index: 20; pointer-events: none; box-shadow: 0 4px 16px rgba(0,0,0,0.3); }
  .renju-app .info-tip:hover .info-tip-text { visibility: visible; opacity: 1; }
  @media (max-width: 1080px) {
    .renju-app { --board-size: min(82vw, 680px); }
    .renju-app .wrap { grid-template-columns: 1fr; max-width: 760px; }
  }
</style>
...
<div class="wrap">
  <div class="panel">
    <h1>Renju Dojo 連珠道場 v5.4</h1>
    <div class="row">
      <button id="newGameBtn" class="primary">New game</button>
      <button id="undoBtn">Undo</button>
      <button id="switchBtn">Switch sides</button>
      <button id="engineMoveBtn">Engine move now</button>
      <select id="humanColorSel">
        <option value="1" selected>Human plays Black</option>
        <option value="2">Human plays White</option>
      </select>
      <select id="timeSel">
        <option value="1000">Fast 1000 ms</option>
        <option value="2500" selected>Normal 2500 ms</option>
        <option value="8000">Long 8000 ms</option>
      </select>
    </div>
    <div id="resultBanner" class="resultBanner" aria-live="polite"></div>
    <div class="boardBox" style="margin-top:14px;"><canvas id="board" width="760" height="760"></canvas></div>
  </div>
  <div class="panel">
    <div class="infoGrid">
      <div class="card"><div class="label">Turn</div><div id="turnVal" class="value">Black</div></div>
      <div class="card"><div class="label">Result</div><div id="resultVal" class="value">Playing</div></div>
      <div class="card"><div class="label">Engine</div><div id="engineVal" class="value">Idle</div></div>
      <div class="card"><div class="label">Nodes</div><div id="nodesVal" class="value">0</div></div>
    </div>
    <div style="margin-top:14px; display:flex; align-items:center; justify-content:space-between;">
      <span style="font-weight:700; display:flex; align-items:center; gap:6px;">Winning Rate <span class="info-tip">ℹ<span class="info-tip-text">Calculated by the engine using the thinking time set above. A longer thinking time gives a deeper search and a more accurate win rate estimate.</span></span></span>
      <button id="winRateToggleBtn" style="padding:5px 10px;font-size:12px;">On</button>
    </div>
    <div id="winRatePanel" class="winRateBox">
      <div>
        <div class="winRateBar" aria-label="Winning rate bar">
          <div id="winRateBlack" class="winRateFillBlack" style="width:50%"></div>
          <div id="winRateWhite" class="winRateFillWhite" style="width:50%"></div>
        </div>
        <div class="winRateLegend">
          <span id="winRateBlackLabel">Black 50%</span>
          <span id="winRateWhiteLabel">White 50%</span>
        </div>
        <div id="winRateVal" class="winRateValue">Even</div>
      </div>
    </div>
    <div style="margin-top:12px; display:flex; align-items:center; justify-content:space-between;">
      <span style="font-weight:700;">Top candidates</span>
      <button id="candidatesToggleBtn" style="padding:5px 10px;font-size:12px;">Off</button>
    </div>
    <div id="cand" class="cand hidden">No search yet.</div>
    <div id="errBox" class="cand" style="margin-top:10px;color:#ffb4b4;"></div>
  </div>
</div>
</div>
